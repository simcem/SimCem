...
namespace simcem
{
  ...
  public:
    ...
    Base::const_iterator find(std::string key) const
    {
      ...
      for (auto it = this->begin(); it != this->end(); ++it)
      {
        ...
        if (ID not_eq trial_ID)
          continue;
        if (not phase_ID.empty() and phase_ID not_eq trial_phase_ID)
          continue;
        if (not src_ID.empty() and src_ID not_eq trial_src_ID)
          continue;
        return it;
      }
  ...
  */
  ...
  struct Isotope
  {
    Isotope(size_t Z, size_t N, double mass, double mass_uncertainty, double abundance, std::string symbol, std::string name, std::string category) : _Z(Z), _N(N), _mass(mass), _mass_uncertainty(mass_uncertainty), _abundance(abundance), _symbol(symbol), _name(name), _category(category)
    {
    }

    Isotope(Node xml, std::string symbol, std::string name, std::string category) : _Z(xml.getAttribute("Z").as<size_t>()),
  ...
  struct Element : public std::vector<Isotope>, Isotope
  {
    ...
    Element(size_t Z, size_t N, double mass, double mass_uncertainty, double abundance, std::string symbol, std::string name, std::string category, Base isotopes, size_t group, size_t period, std::string block = "", std::string referenceComponentID = "") : Isotope(Z, N, mass, mass_uncertainty, abundance, symbol, name, category),
                                                                                                                                                                                                                                                                 Base(isotopes),
                                                                                                                                                                                                                                                                 _group(group),
                                                                                                                                                                                                                                                                 _period(period),
                                                                                                                                                                                                                                                                 _block(block),
    ...
    Element(Node xml) : Isotope(xml, xml.getAttribute("Symbol"), xml.getAttribute("Name"), xml.getAttribute("Category")),
  ...
  struct Isobar
  {
    Isobar(shared_ptr<Curve> curve, double p) : security("Public"),
                                                _curve(curve),
                                                _p(p)
    {
    }

    Isobar(Node xml) : security("Public"),
  ...
  struct Phase : public std::vector<shared_ptr<Isobar>>
  {
    ...
    Phase(std::string name, std::string type, std::shared_ptr<Component> md) : _name(name), _type(type),
                                                                               _mol(md)
    {
    }
    ...
    {
      _aliases.erase(std::remove_if(_aliases.begin(), _aliases.end(), [alias](std::string a)
                                    { return boost::iequals(alias, a); }),
                     _aliases.end());
    }
  ...
  struct Bond
  {
    Bond(shared_ptr<Atom> atom1, shared_ptr<Atom> atom2, double order) : _atom1(atom1), _atom2(atom2), _order(order)
    {
    }
  ...
  struct Atom
  {
    Atom() : _ID(0), _Z(0), _N(0), _charge(0), _x(0), _y(0)
    {
    }

    Atom(size_t ID, size_t Z, int N, double x, double y, double charge) : _ID(ID), _Z(Z), _N(N), _charge(charge), _x(x), _y(y)
    {
    }

    Atom(Node xml) : _ID(xml.getAttribute("ID").as<size_t>()),
    ...
    {
      return (_ID == a._ID) && (_Z == a._Z) && (_N == a._N) && (_charge == a._charge) && (_bonds == a._bonds);
    }
  ...
  struct Component : public std::vector<Phase>, std::enable_shared_from_this<Component>
  {
    ...
    {
      Property(Node xml) : Data(xml),
                           _value(xml.getAttribute("Value").as<double>()),
                           _type(xml.getName())
      {
      }

      Property(Property_t type, double v, std::string source, T_unit_t Tunit = T_unit_t::K, Q_unit_t Qunit = Q_unit_t::mol, E_unit_t Eunit = E_unit_t::J, P_unit_t Punit = P_unit_t::Pa, L_unit_t Lunit = L_unit_t::m) : Data("", source, Tunit, Qunit, Eunit, Punit, Lunit),
                                                                                                                                                                                                                         _value(v),
                                                                                                                                                                                                                         _type(type)
      {
      }
      ...
      {
        return (_value == p._value) && (_type == p._type) && Data::operator==(p);
      }
  ...
  private:
    Component(std::string ID, Components elements, const Database &db, std::string formula = "") : _elements(elements),
                                                                                                   _formula(formula),
                                                                                                   _identifier(ID)
    {
      if (_formula.empty())
  ...
  struct IsobarIterator
  {
    ...
    IsobarIterator(const outer_iterator it) : _out_it(it),
                                              _out_end(it)
    {
    }
  ...
  struct IsobarRange : std::vector<shared_ptr<Isobar>>
  {
    ...
    IsobarRange(const Database &db, std::string ID, PhaseFilters &&pfilters = {}, IsobarFilters &&ifilters = {}) : _phaseFilters(pfilters),
                                                                                                                   _isobarFilters(ifilters)
    {
      ...
      if (!phaseID.empty())
        _phaseFilters.push_back([=](const Phase &phase)
                                { return phaseID != phase._name; });
      if (!sourceID.empty())
        _isobarFilters.push_back([=](const shared_ptr<Isobar> &isobar)
                                 { return isobar->getCurve()->_source != sourceID; });

      // First pass to count the number of isobars
      size_t isobar_count = 0;
      forEachIsobar(mol, [&](const shared_ptr<Isobar> &)
                    { ++isobar_count; });
      ...
      isobar_count = 0;
      forEachIsobar(mol, [&](const shared_ptr<Isobar> &isobar)
                    { (*this)[isobar_count++] = isobar; });
    }
  ...
  {
  public:
    Model(shared_ptr<Database> db, Components components, const std::string type) : Components(components),
                                                                                    _db(db), _type(type)
    {
    }
  ...
  protected:
    ...
    struct IsoIt : IsobarIterator
    {
      ...
      const Database &_db;
      IsoIt(const Database &db, const std::string ID, const std::string type) : _type(type),
                                                                                _db(db)
      {
      ...
      virtual bool skip_outer()
      {
        return (_out_it->_type != _type) || (!_phaseID.empty() && (_phaseID != _out_it->_name));
      }

      virtual bool skip_inner()
      {
        return ((*_in_it)->getCurve()->getVariable() != Objective_t::G) || (!_sourceID.empty() && (_sourceID != (*_in_it)->getCurve()->_source));
      }
    };
  ...
  public:
    ModelExcess(shared_ptr<Model> model) : Model(model->db(), Components(*model), model->type()),
                                           _base_model(model)
    {
    }
...
#define fail() stator_throw() << "Cannot use an excess model this way!";
  ...
  {
  public:
    ModelT(shared_ptr<Database> db, Components components, const double T, const std::string type) : Model(db, components, type),
                                                                                                     _T(T)
    {
    }
  ...
  */
  ...
  {
  public:
    ModelTp(shared_ptr<Database> db, Components components, const double T, const double p, const std::string type) : ModelT(db, components, T, type),
                                                                                                                      _p(p)
    {
    }
  ...
  */
  ...
  {
  public:
    ModelIdealGasTp(shared_ptr<Database> db, Components components, const double T, const double p) : ModelTp(db, components, T, p, "idealgas")
    {
    }
  ...
  */
  ...
  {
  public:
    ModelTV(shared_ptr<Database> db, Components components, const double T, const double V, const std::string type) : ModelT(db, components, T, type),
                                                                                                                      _V(V)
    {
    }
  ...
  */
  ...
  {
  public:
    ModelIdealGasTV(shared_ptr<Database> db, Components components, const double T, const double V) : ModelTV(db, components, T, V, "idealgas")
    {
    }
  ...
  {
  public:
    ModelIncompressible(shared_ptr<Database> db, Components components, const double T, const double p, const std::string type, bool mixingEntropy = true) : ModelTp(db, components, T, p, type),
  ...
  {
  public:
    ModelIncompressibleSolid(shared_ptr<Database> db, Components components, const double T, const double p) : ModelIncompressible(db, components, T, p, "solid", false)
    {
    }
  };
  ...
  {
  public:
    ModelIncompressibleLiquid(shared_ptr<Database> db, Components components, const double T, const double p) : ModelIncompressible(db, components, T, p, "liquid", true)
    {
    }
  };
  ...
  */
    ...
    */
    void equilibrate(double initialY1 = HUGE_VAL, double initialY2 = HUGE_VAL)
    {
      try
      {
        ...
        if (_debug)
          std::cout << "Molar constraint vector before elimination :" << std::endl
                    << _molar_constraint_grad << std::endl;
        ...
        if (_rank_reduce_material_constraints)
        {
          ...
          if (_debug)
            std::cout << "Reduced constraints matrix :" << std::endl
                      << _molar_constraint_grad << std::endl;
        }
        else if (_debug)
        {
...
#ifdef SIMCEM_NLOPT
        {
          ...
          try
          {
            ...
            if (_debug)
              std::cout << "Optimiser halted successfully:" << NLOPT_exit_code(r) << std::endl;
          }
        ...
        case Optimiser_t::IPopt:
        {
          ...
          {
            NLProblem(System &sys, const Eigen::Matrix<double, Eigen::Dynamic, 1> &g0) : _sys(sys),
                                                                                         _g0(g0)
            {
  ...
  private:
    ...
    static std::string NLOPT_exit_code(nlopt::result v)
    {
      ...
      case nlopt::FTOL_REACHED:
        return "F tolerance reached";
      case nlopt::XTOL_REACHED:
        return "X tolerance reached";
      case nlopt::MAXEVAL_REACHED:
        return "Max evaluations reached";
  ...
  namespace trans
  {
     ...
     */
    inline std::pair<double, double> NASA_transport(const shared_ptr<Model> phase)
    {
      ...
      for (const Components::value_type &i : *phase)
      {
        IsobarRange isobars(db, i.first,
                            {[](const Phase &phase)
                             { return phase._type != "idealgas"; }},
                            {[](const shared_ptr<Isobar> &isobar)
                             { return ((isobar->getCurve()->getVariable() != Objective_t::dynvisc) && (isobar->getCurve()->getVariable() != Objective_t::thermcond)) || (isobar->getCurve()->_source != "NASA_GLENN"); }});
        double visc = 0;
        double thermcond = 0;
        sym::Var<sym::vidx<'T'>> varT;
