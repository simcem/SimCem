...
class CoreTest(unittest.TestCase):
    ...
    def test_phase_properties(self):
        phase1 = ModelIdealGasTp(self.db, Components({"CO2":1.0}), 298.15, 1.01325e5) 
        phase2 = ModelIdealGasTV(self.db, Components({"CO2":1.0}), 298.15, phase1.V())
    ...
    def test_phase_properties_NASA_methane(self):
        ...
        phase1 = ModelIdealGasTp(self.db, Components({"CH4":1.0}), 350, 80e5)
        phase2 = ModelIdealGasTV(self.db, Components({"CH4":1.0}), 350, phase1.V())
        #CEA output is in kj/kg, need molar mass to convert
        molar_mass = Components({"CH4":1.0}).M(self.db)
    ...
    def test_phase_properties_NASA_mixture(self):
        ...
        n_CH4 = 1 / Components({"CH4":1.0}).M(self.db)
        n_O2 = 1 / Components({"O2":1.0}).M(self.db)
        phase1 = ModelIdealGasTp(self.db, Components({"CH4":n_CH4, "O2":n_O2}), 350, 80e5)
        phase2 = ModelIdealGasTV(self.db, Components({"CH4":n_CH4, "O2":n_O2}), 350, phase1.V())
        molar_mass = Components({"CH4":n_CH4, "O2":n_O2}).M(self.db)
...
##        
##        A = ModelIdealGasTV(self.db, Components({"N2":1.0}), 298.15, 10)
##        B = MyPhaseClassTV(self.db, Components({"N2":1.0}), 298.15, 10)
##        
...
##        self.assertEqual(B.dpdT_counter, 2)
    ...
    def test_Tp_water_flash(self):
        ...
        for reactive in [True,False]:
            ...
            A = ModelIdealGasTp(self.db, Components({"H2O":1}), 273.15+50, 1.01325e5)
            B = ModelIncompressibleLiquid(self.db, Components({"H2O":1}), 273.15+50, 1.01325e5)
            sys.append(A)
            sys.append(B)
            sys.equilibrate()
...
##                self.assertAlmostEqual(-sys.lagrangians["H2O"]/sys[1].chemPot("H2O"), 1, places=9)
    ...
    def test_pH_water_flash(self):
        ...
        GasH = ModelIdealGasTp(self.db, Components({"H2O":1}), 273.15+100, 1.01325e5).H()
        LiqH = ModelIncompressibleLiquid(self.db, Components({"H2O":1}), 273.15+100, 1.01325e5).H()

        A = ModelIdealGasTp(self.db, Components({"H2O":0}), 273.15+100, 1.01325e5)
        B = ModelIncompressibleLiquid(self.db, Components({"H2O":1}), 273.15+100, 1.01325e5)
        sys.append(A)
        sys.append(B)
...
##        self.assertAlmostEqual(sys.lagrangians["H2O"]/lagrangian, 1, places=7)
    ...
    def test_gas_Tp_combustion(self):
        ...
        A = ModelIdealGasTp(self.db, Components({"N2":0.79, "O2":0.21, "CH4":0.105, "CO2":0.0, "CO":0.0, "H2O":0.0}), 2600, 1.01325e5)
        sys.append(A)
        ...
        sys.equilibrate()
        testResult = ModelIdealGasTp(self.db, Components({"CH4":8.688777086942181e-12, "CO":0.03453317988062815, "CO2":0.07046682011068307, "H2O":0.2099999999826224, "N2":0.79, "O2":0.01726658995769161}), 2600, 1.01325e5);
    ...
    def test_gas_TV_combustion(self):
        ...
        A = ModelIdealGasTV(self.db, Components({"N2":0.79, "O2":0.21, "CH4":0.105, "CO2":0.0, "CO":0.0, "H2O":0.0}), 2600, V)
        sys.append(A)
        sys.equilibrate()
        testResult = ModelIdealGasTp(self.db, Components({"CH4":8.678512425246362e-12, "CO":0.03453322192945054, "CO2":0.07046677806187095, "H2O":0.209999999982643, "N2":0.79, "O2":0.01726661098208228}), 2600, 1.01325e5);
        result = testResult.components - A.components
        result.removeSmallComponents(1e-7)
        self.assertEqual(len(result), 0, msg="remainder "+str(result))
    ...
    def test_solidgas_Tp_combustion(self):
        ...
        sys.append(B)
        sys.equilibrate()
        testResult = ModelIdealGasTp(self.db, Components({"CO":0.1760714215970302, "CO2":0.823928578398245, "O2":0.08803571080323989}), 2600, 1.01325e5);
        result = testResult.components - A.components
    ...
    def test_solidgas_TV_combustion(self):
        ...
        sys.append(B)
        sys.equilibrate()
        testResult = ModelIdealGasTp(self.db, Components({"CO":0.1760715474833369, "CO2":0.823928452512056, "O2":0.08803577374627462}), 2600, 1.01325e5);
        result = testResult.components - A.components
        result.removeSmallComponents(1e-6)
        self.assertEqual(len(result), 0, msg="remainder "+str(result))
